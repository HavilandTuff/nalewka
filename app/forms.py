...
from app import db
from app.models import Ingredient, Batch, Liquor, User
...
class BatchFormulaForm(FlaskForm):
    batch_description = TextAreaField('Batch Description', validators=[
        DataRequired(), 
    ...
    bottle_count = IntegerField('Number of Bottles', validators=[
        NumberRange(min=0, message='Bottle count must be 0 or greater')
    ])
    bottle_volume = FloatField('Bottle Volume', validators=[
        NumberRange(min=0.1, message='Bottle volume must be greater than 0')
    ])
    bottle_volume_unit = SelectField('Volume Unit', choices=[
        ('ml', 'Milliliters'), 
        ('l', 'Liters')
    ], default='ml')
    
    # Dynamic ingredients list
    ingredients = FieldList(FormField(IngredientEntryForm), min_entries=1, max_entries=20)
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user_id:
            self.liquor.choices = [(0, 'Select a liquor...')] + [
                (liquor.id, liquor.name) for liquor in Liquor.query.filter_by(user_id=user_id).all()
            ]
        else:
            self.liquor.choices = [(0, 'Select a liquor...')]
    ...
    def validate_ingredients(self, field):
        ...
        for ingredient_form in field.data:
            if (ingredient_form.get('ingredient') and 
                ingredient_form.get('ingredient') != 0 and
                ingredient_form.get('quantity') and 
                ingredient_form.get('unit')):
                valid_ingredients += 1
