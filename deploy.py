...
def create_sample_data():
    ...
    with app.app_context():
        ...
        liquors = [
            Liquor(name='Wiśniówka', description='Traditional Polish cherry liqueur made with fresh cherries and vodka', user_id=admin_user.id),
            Liquor(name='Cytrynówka', description='Refreshing lemon liqueur with citrus zest and honey', user_id=admin_user.id),
            Liquor(name='Nalewka z Malin', description='Sweet raspberry liqueur perfect for desserts', user_id=admin_user.id),
        ]
        ...
        formulas = [
            BatchFormula(batch_id=batch.id, ingredient_id=ingredients[0].id, quantity=1000, unit='grams'),
            BatchFormula(batch_id=batch.id, ingredient_id=ingredients[3].id, quantity=300, unit='grams'),
            BatchFormula(batch_id=batch.id, ingredient_id=ingredients[5].id, quantity=1500, unit='milliliters'),
        ]
